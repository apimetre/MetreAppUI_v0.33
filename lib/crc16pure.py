...
def _crc16_update(byte, crc, table):
    return ( ( ((crc<<8)&0xff00) ^ table[((crc>>8)&0xff)^byte] ) & 0xffff )
...
def crc16xmodem_update(data, crc=0):
    return _crc16_update(data, crc, CRC16_XMODEM_TABLE)
